...
public class SecuritySuccessHandler implements AuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if (datosUsuario != null) {
			...
			redirectStrategy.sendRedirect(request, response, "http://localhost:3000/restaurantes");
	...
	private Map<String, String> fetchUserInfo(String oauthID) {
		...
		try {
			Usuario usuario = servicio.findByOAuthId(oauthID);
...
//	public ResponseEntity getLogin(String key, String subject) {
//        try {
//            HttpHeaders headers = new HttpHeaders();
//            headers.setLocation(URI.create(authService.redirectAuth(key, subject)));
